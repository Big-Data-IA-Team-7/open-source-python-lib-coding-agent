...
def login(request: LoginRequest):
    ...
    try:
        ...
        if not request.username or not request.password:
            logger.error(f"Missing credentials - Username provided: {bool(request.username)}, Password provided: {bool(request.password)}")
            raise HTTPException(
        ...
        logger.info(f"User fetch result: {user is not None}")
        
        if user is None or user.empty:
            logger.info(f"User not found or empty DataFrame for username: {request.username}")
            raise HTTPException(
        ...
        try:
            ...
            logger.error(f"Error accessing user data structure: {str(e)}")
            raise HTTPException(
